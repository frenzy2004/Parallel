import { app, BrowserWindow, desktopCapturer, globalShortcut, ipcMain, screen, systemPreferences, } from "electron";
...
let activeLassoBounds = null;
const loadView = async (window, view) => {
    if (rendererUrl) {
...
const openCapture = async () => {
    ...
    if (access === "denied" || access === "restricted") {
        throw new Error("Screen recording access is required in System Settings → Privacy & Security.");
    }
    ...
    captureWindow.webContents.send("parallel:capture-source", source.thumbnail.toDataURL());
    captureWindow.show();
    captureWindow.focus();
};
const showSidecar = async (cropDataUrl, lassoBounds) => {
...
const showMapping = async (anchorIds) => {
...
ipcMain.handle("parallel:submit-crop", async (_event, payload) => showSidecar(payload.cropDataUrl, payload.bounds));
ipcMain.handle("parallel:set-mapping-highlights", async (_event, anchorIds) => showMapping(anchorIds));
