...
describe("ScreenshotImport", () => {
  it("previews one chosen screenshot and submits its bounded in-memory crop", async () => {
    ...
    Object.defineProperties(window, {
      screenX: { configurable: true, value: 300 },
      screenY: { configurable: true, value: 120 },
    });
    render(
      <ScreenshotImport
    ...
    await waitFor(() =>
      expect(onSubmit).toHaveBeenCalledWith(
        ...
        { x: 324, y: 216, width: 412, height: 309 },
      ),
    );
    expect(await screen.findByText("Original screenshot")).toBeInTheDocument();
    expect(
      screen.getByText(/keep this window visible for map highlights/i),
    ).toBeInTheDocument();
    expect(
      screen.queryByRole("button", { name: "Make twin" }),
