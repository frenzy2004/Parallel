...
- Produces: `fitImportedCrop(imageSize, displayBounds): Rectangle`

- [ ] **Step 1: Write failing tests**

Cover one permitted raster, empty/multiple files, SVG/text, empty and
over-8-MiB files, read failure, and a hand-derived centered crop rectangle that
stays within a display with a non-zero origin.
...
aspect-fit math clamped to the display. Do not inspect or expose filesystem
paths.
...
file only in memory and forward its data URL.
